...
export default function StoryOne() {
  ...
  return (
    <main ref={containerRef} className="bg-[var(--color-ink)] text-[var(--color-paper)] min-h-screen">
      <div className="grain-overlay"></div>
      
      {/* UI CHROME - Story 1: #00F0FF nav, solid, no mix blend */}
      <nav className="fixed top-0 left-0 w-full p-8 z-50 flex justify-between text-[12px] uppercase tracking-widest text-[#00F0FF] pointer-events-none">
      ...
      <div className="w-full">
        {storyData.scenes.map((scene) => {
          ...
          return (
            <section 
              key={scene.id} 
              className="scene-track relative border-b border-white/10"
            ...
            >
              ...
              <div className="scene-stage relative flex flex-col items-center justify-center w-full px-8"
                ...
                <h2 className="text-[#00F0FF] text-[10px] uppercase tracking-widest absolute top-8 left-8 opacity-30">
                ...
                {scene.captions.map((cap, j) => (
                  <p 
                    ...
                    style={{
                      ...
                      color: cap.speaker ? 'var(--color-s1-amber)' : 'var(--color-paper)'
                    }}
                  >
                    {cap.text}
                  </p>
